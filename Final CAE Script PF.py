...
SP=100     #Tie-Bar bar spacing as 'mm'
...
folder_path = r'F:\MSc\Post-Fire-'+str(name)+'-R-'+str(r3)+'mm'
model_name = str(name)
...
mdb.models['Model-1'].rootAssembly.Set(name='U', vertices=mdb.models['Model-1'].rootAssembly.instances['Beam-1'].vertices.findAt(((0.5*b-(0.5*r3+r1+Cs), h-(0.5*r3+r1+Cb), -0.5*L), )))
mdb.models['Model-1'].rootAssembly.Set(name='TC-B', vertices=mdb.models['Model-1'].rootAssembly.instances['Beam-1'].vertices.findAt(((0.5*b-(0.5*r3+r1+Cs), -(0.5*r3+r1+Cb), -0.5*L), )))
...
mdb.models['Model-1'].FieldOutputRequest(createStepName='PF-Load', name='F-Output-2', variables=('PE', 'U', 'RF', 'S', 'PEMAG'))
...
mdb.models['Model-1'].FilmCondition(createStepName='Fire', definition=
    PROPERTY_REF, interactionProperty='NF', name='Slab', sinkAmplitude='F-'+str(t1), 
    sinkDistributionType=UNIFORM, sinkFieldName='', sinkTemperature=1.0, 
...
mdb.models['Model-1'].RigidBody(name='S1', refPointRegion=
    ...
    faces=mdb.models['Model-1'].rootAssembly.instances['Beam-1'].faces.findAt(
    ((0.5*b-(0.5*r3+r1+Cs)-5, -0.5*r3-r1-Cb, -S1-5), ), ((0.5*b-(0.5*r3+r1+Cs)+5, -0.5*r3-r1-Cb, -S1-5), ), ((0.5*b-(0.5*r3+r1+Cs)-5, -0.5*r3-r1-Cb, -S1+5), ), ((0.5*b-(0.5*r3+r1+Cs)+5, -0.5*r3-r1-Cb, -S1+5), ), )))
    
mdb.models['Model-1'].RigidBody(name='S2', refPointRegion=
    mdb.models['Model-1'].rootAssembly.sets['RP4'], tieRegion=Region(
    faces=mdb.models['Model-1'].rootAssembly.instances['Beam-1'].faces.findAt(
    ((0.5*b-(0.5*r3+r1+Cs)-5, -0.5*r3-r1-Cb, -S2-5), ), ((0.5*b-(0.5*r3+r1+Cs)+5, -0.5*r3-r1-Cb, -S2-5), ), ((0.5*b-(0.5*r3+r1+Cs)-5, -0.5*r3-r1-Cb, -S2+5), ), ((0.5*b-(0.5*r3+r1+Cs)+5, -0.5*r3-r1-Cb, -S2+5), ), )))
...
mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='PF-Load', 
    ...
    -200, u3=UNSET, ur1=UNSET, ur2=UNSET, ur3=UNSET)
mdb.models['Model-1'].DisplacementBC(amplitude=UNSET, createStepName='PF-Load', 
    distributionType=UNIFORM, fieldName='', fixed=OFF, localCsys=None, name=
...
mdb.models['Model-1'].parts['Beam'].seedEdgeBySize(constraint=FINER, 
    ...
    '[#c2fffc0b #ffffffff #7fff80ab #ef007f80 #fe16fff9 #e07ff ]', ), ), size=
    2*m)
mdb.models['Model-1'].parts['Beam'].seedEdgeBySize(constraint=FINER, 
    deviationFactor=0.1, edges=
    mdb.models['Model-1'].parts['Beam'].edges.getSequenceFromMask((
    '[#2c000370 #0 #80007800 #17003d #600006 #1e000 ]', ), ), size=m)
...
mdb.models['Model-1'].rootAssembly.Set(name='TC-R', nodes=mdb.models['Model-1'].rootAssembly.instances['Main-Bar-1-lin-1-2'].nodes[start_index:end_index])
mdb.models['Model-1'].rootAssembly.Set(name='TC-M', nodes=mdb.models['Model-1'].rootAssembly.instances['Main-Bar-1-lin-2-1'].nodes[start_index:end_index])
mdb.models['Model-1'].rootAssembly.Set(name='TC-T', nodes=mdb.models['Model-1'].rootAssembly.instances['Top-Bar-1-lin-1-2'].nodes[start_index:end_index])

#del mdb.models['Model-1'].historyOutputRequests['H-Output-1']
del mdb.models['Model-1'].historyOutputRequests['H-Output-1']
mdb.models['Model-1'].HistoryOutputRequest(createStepName='PF-Load', name='Displacement', rebar=EXCLUDE, region=mdb.models['Model-1'].rootAssembly.sets['U'], sectionPoints=DEFAULT, variables=('U2', ))
mdb.models['Model-1'].HistoryOutputRequest(createStepName='Fire', name='Temperature', rebar=EXCLUDE, region=mdb.models['Model-1'].rootAssembly.sets['TC-R'], sectionPoints=DEFAULT, variables=('NT', ))
mdb.models['Model-1'].historyOutputRequests['Temperature'].deactivate('PF-Load')
mdb.models['Model-1'].HistoryOutputRequest(createStepName='Fire', name='Temperature-2', rebar=EXCLUDE, region=mdb.models['Model-1'].rootAssembly.sets['TC-M'], sectionPoints=DEFAULT, variables=('NT', ))
mdb.models['Model-1'].historyOutputRequests['Temperature-2'].deactivate('PF-Load')
mdb.models['Model-1'].HistoryOutputRequest(createStepName='Fire', name='Temperature-3', rebar=EXCLUDE, region=mdb.models['Model-1'].rootAssembly.sets['TC-T'], sectionPoints=DEFAULT, variables=('NT', ))
mdb.models['Model-1'].historyOutputRequests['Temperature-3'].deactivate('PF-Load')
mdb.models['Model-1'].HistoryOutputRequest(createStepName='Fire', name='Temperature-4', rebar=EXCLUDE, region=mdb.models['Model-1'].rootAssembly.sets['TC-B'], sectionPoints=DEFAULT, variables=('NT', ))
mdb.models['Model-1'].historyOutputRequests['Temperature-4'].deactivate('PF-Load')
mdb.models['Model-1'].HistoryOutputRequest(createStepName='PF-Load', name='Force', rebar=EXCLUDE, region=mdb.models['Model-1'].rootAssembly.sets['RP3'], sectionPoints=DEFAULT, variables=('RF2', ))
...
mdb.Model(name=str(name)+'-F-'+str(t7), objectToCopy=mdb.models[str(name)+'-F-'+str(t1)])
mdb.Model(name=str(name)+'-F-'+str(t8), objectToCopy=mdb.models[str(name)+'-F-'+str(t1)])
mdb.Model(name=str(name)+'-F-'+str(t9), objectToCopy=mdb.models[str(name)+'-F-'+str(t1)])

t = [t2, t3, t4, t5, t6, t7, t8]  # Assuming t2, t3, ..., t8 are defined elsewhere
...
del mdb.models[str(name)+'-F-'+str(t9)].steps['Fire']
...
mdb.models[str(name)+'-F-'+str(t9)].Material(name='Steel-Material-20')
mdb.models[str(name)+'-F-'+str(t9)].materials['Steel-Material-20'].Density(table=((7.85e-09, ), ))
mdb.models[str(name)+'-F-'+str(t9)].materials['Steel-Material-20'].Plastic(table=ms20, temperatureDependency=OFF)
mdb.models[str(name)+'-F-'+str(t9)].materials['Steel-Material-20'].Elastic(table=((E*1000, Ve), ), temperatureDependency=OFF)
...
Stb = np.concatenate((Fc * ((2*(e[1:]/epc20))/(1+(e[1:]/epc20)**2)), [0]))
et = np.concatenate(([0], e[1:]-((St[1:]*epc20)/(2*Fc)))) 

for s, e_val in zip(St, et):
        mc20 +=((round(s, 5), round(e_val, 5)),)

for s, s1, e_val in zip(St, Stb, et):
        mcdc20 += ((round(0 if s - s1 < 0 else (1 - (s / Fc)), 5), round(e_val, 5)),)
...
Stb = np.array([0.1*0.77*Fc, 0.1*0.45*Fc, 0.1*0.1*Fc, 0])
...
for s, s1, e_val in zip(St, Stb, et):
        mcdt20 += ((round(0 if s - s1 < 0 else (1 - (s / (0.1*Fc))), 5), round(e_val, 5)),)

mdb.models[str(name)+'-F-'+str(t9)].Material(name='Concrete-Material-20')
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].Density(table=((2.4e-09, ), ))
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].Elastic(table=mce20, temperatureDependency=OFF)
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].ConcreteDamagedPlasticity(table=((31.0, 0.1, 1.16, 0.667, 0.0001), ))
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].concreteDamagedPlasticity.ConcreteCompressionHardening(table=mc20, temperatureDependency=OFF)
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].concreteDamagedPlasticity.ConcreteCompressionDamage(table=mcdc20, temperatureDependency=OFF)
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].concreteDamagedPlasticity.ConcreteTensionStiffening(table=mct20, temperatureDependency=OFF)
mdb.models[str(name)+'-F-'+str(t9)].materials['Concrete-Material-20'].concreteDamagedPlasticity.ConcreteTensionDamage(table=mcdt20, temperatureDependency=OFF)

mdb.models[str(name)+'-F-'+str(t9)].sections['Beam'].setValues(material='Concrete-Material-20', thickness=None)
mdb.models[str(name)+'-F-'+str(t9)].sections['Main-Bar'].setValues(area=A3, material='Steel-Material-20')
mdb.models[str(name)+'-F-'+str(t9)].sections['Tie-Bar'].setValues(area=A1, material='Steel-Material-20')
mdb.models[str(name)+'-F-'+str(t9)].sections['Top-Bar'].setValues(area=A2, material='Steel-Material-20')

mdb.models[str(name)+'-F-'+str(t9)].parts['Beam'].setElementType(elemTypes=(
    ElemType(elemCode=C3D8R, elemLibrary=STANDARD, secondOrderAccuracy=OFF, 
    kinematicSplit=AVERAGE_STRAIN, hourglassControl=DEFAULT, 
    distortionControl=DEFAULT), ElemType(elemCode=C3D6, elemLibrary=STANDARD), 
    ElemType(elemCode=C3D4, elemLibrary=STANDARD)),regions=(
    mdb.models[str(name)+'-F-'+str(t9)].parts['Beam'].cells.findAt(
    ((0.5*b-5, h, S1-5), ), ((0.5*b+5, h, S1-5), ), ((0.5*b-5, 0, S1-5), ), ((0.5*b+5, 0, S1-5), ), 
    ((0.5*b-5, h, S1+5), ), ((0.5*b+5, h, S1+5), ), ((0.5*b-5, 0, S1+5), ), ((0.5*b+5, 0, S1+5), ),
    ((0.5*b-5, h, S2-5), ), ((0.5*b+5, h, S2-5), ), ((0.5*b-5, 0, S2-5), ), ((0.5*b+5, 0, S2-5), ), 
    ((0.5*b-5, h, S2+5), ), ((0.5*b+5, h, S2+5), ), ((0.5*b-5, 0, S2+5), ), ((0.5*b+5, 0, S2+5), ),
...
mdb.models[str(name)+'-F-'+str(t9)].parts['Main-Bar'].setElementType(elemTypes=(
    ElemType(elemCode=T3D2, elemLibrary=STANDARD), ), regions=(
    mdb.models[str(name)+'-F-'+str(t9)].parts['Main-Bar'].edges.findAt(((0.5*L, 0.0, 0.0), )), ))

mdb.models[str(name)+'-F-'+str(t9)].parts['Tie-Bar'].setElementType(elemTypes=(
    ElemType(elemCode=T3D2, elemLibrary=STANDARD), ), regions=(
    mdb.models[str(name)+'-F-'+str(t9)].parts['Tie-Bar'].edges.findAt(((0.5*(b-2*Cs-r1), 0.0, 0.0), ), ((0.5*(b-2*Cs-r1), h-Ct-Cb-r1, 0.0), ), 
    ((0.0, 0.5*(h-Ct-Cb-r1), 0.0), ), ((b-2*Cs-r1, 0.5*(h-Ct-Cb-r1), 0.0), ), ), ))

mdb.models[str(name)+'-F-'+str(t9)].parts['Top-Bar'].setElementType(elemTypes=(
    ElemType(elemCode=T3D2, elemLibrary=STANDARD), ), regions=(
    mdb.models[str(name)+'-F-'+str(t9)].parts['Top-Bar'].edges.findAt(((0.5*L, 0.0, 0.0), )), ))

mdb.models[str(name)+'-F-'+str(t9)].rootAssembly.regenerate()
mdb.models[str(name)+'-F-'+str(t9)].fieldOutputRequests['F-Output-2'].setValues(
    variables=('S', 'PE', 'VEEQ', 'PEMAG', 'LE', 'U', 'RF', 'CF', 'CSTRESS', 'CDISP', 'DAMAGEC', 'DAMAGET'))

#Job

a = [t9, t1, t2, t3, t4, t5, t6, t7, t8]
...
times1 = [t1, t2, t3, t4, t5, t6, t7, t8, t9]

report_path = 'F:\MSc\Post-Fire-'+ str(name) +'-R-'+ str(r3) +'mm' + '\'
...
for a in times:
	...
	xy_result1 = session.XYData(name='T-' + str(a) + '-min-R', objectToCopy=xy6)
	xy7 = xyPlot.XYDataFromHistory(odb=odb, outputVariableName='Nodal temperature: NT11 PI: MAIN-BAR-1-LIN-2-1 Node 65 in NSET TC-M', steps=('Fire',))
	xy_result1 = session.XYData(name='T-' + str(a) + '-min-M', objectToCopy=xy7)    
	xy8 = xyPlot.XYDataFromHistory(odb=odb, outputVariableName='Nodal temperature: NT11 PI: TOP-BAR-1-LIN-1-2 Node 65 in NSET TC-T', steps=('Fire',))
	xy_result1 = session.XYData(name='T-' + str(a) + '-min-T', objectToCopy=xy8)     
	xy9 = xyPlot.XYDataFromHistory(odb=odb, outputVariableName='Nodal temperature: NT11 PI: BEAM-1 Node 11 in NSET TC-B', steps=('Fire',))
	xy_result1 = session.XYData(name='T-' + str(a) + '-min-B', objectToCopy=xy9)
...
report_path1 = 'C:\Users\DIU\OneDrive - Daffodil International University\MHB-MSc\Post-Fire-' + str(name) +'-R-'+ str(r3) +'mm' + '\Post-Fire-'+ str(name) + '\'
...
# Modify the path when saving CSV files
for a in times:
    data1 = session.xyDataObjects['T-' + str(a) + '-min-R'].data
    data2 = session.xyDataObjects['T-' + str(a) + '-min-M'].data
    data3 = session.xyDataObjects['T-' + str(a) + '-min-T'].data
    data4 = session.xyDataObjects['T-' + str(a) + '-min-B'].data
    
    np.savetxt(report_path1 + 'T-' + str(a) + '-min-R.csv', data1, delimiter=',', fmt='%.2f', header='Time, Temperature')
    np.savetxt(report_path1 + 'T-' + str(a) + '-min-M.csv', data2, delimiter=',', fmt='%.2f', header='Time, Temperature')
    np.savetxt(report_path1 + 'T-' + str(a) + '-min-T.csv', data3, delimiter=',', fmt='%.2f', header='Time, Temperature')
    np.savetxt(report_path1 + 'T-' + str(a) + '-min-B.csv', data4, delimiter=',', fmt='%.2f', header='Time, Temperature')
